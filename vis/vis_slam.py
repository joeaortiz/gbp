...
class MySceneViewer(trimesh.viewer.SceneViewer):
    ...
    def save_viewer(self, save_dir):
        ...
        cv.putText(img,f'ARE: {np.around(self._ares[self.are_ix], 3):.3f} pixels', 
            ...
            fontColor,
            lineType)
        cv.putText(img,f'Ceres converged ARE: {np.round(self.ceres_final_are[self._nkfs_showing - self._nkfs_at_start], 3):.3f} pixels', 
            justBelow, 
            font, 
            fontScale,
            fontColor,
...
def dispbeliefs(res_dir, data_dir):
    ...
    print(len(ceres_final_ares), ceres_final_ares)
